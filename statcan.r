...
#'
#'
#' gets profile for lots of geo_uids
#'
#'
statcan_census_profiles <- function(geo_uids, ...){
  geo_uids %>% 
    map_dfr(function(g){ statcan_census_profile(geo_uid = g, ...)})
...
statcan_geo_uid_from_geo <- function(cpt = "00", geos = "HR", ...){
  ...
  statcan_census_geographies(cpt = cpt, geos = geos, ...) %>%
    pull(geo_uid)
}
